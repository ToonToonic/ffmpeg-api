...
def upload_to_r2(file_path, object_name=None):
    ...
    if not R2_ACCOUNT_ID or not R2_ACCESS_KEY_ID or not R2_SECRET_ACCESS_KEY or not R2_BUCKET:
        print("R2 credentials not configured")
        return f"http://localhost:8000/temp/{object_name}"  # Заглушка для тестирования
    ...
    try:
        ...
        # Замените YOUR_HASH на ваш публичный домен R2
        return f"https://pub-YOUR_HASH.r2.dev/{object_name}"
    except Exception as e:
